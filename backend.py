...
async def handle_line_event(event: dict):
    ...
    if event_type == "message":
        ...
        print(f"🔘 Postback from {user_id}: {data}")
        await handle_postback(user_id, reply_token, data)


async def handle_postback(user_id: str, reply_token: str, data: str):
    """จัดการ postback จาก Rich Menu"""

    # Parse data string เช่น "action=list"
    params = dict(kv.split("=", 1) for kv in data.split("&") if "=" in kv)
    action = params.get("action", "")

    if action == "list":
        await _postback_list(user_id, reply_token)
    elif action == "help":
        await _postback_help(reply_token)
    elif action == "correct_last":
        await _postback_correct_last(reply_token)
    elif action == "delete_last":
        await _postback_delete_last(user_id, reply_token)
    elif action == "summary":
        await _postback_summary(user_id, reply_token)
    elif action == "open_liff":
        liff_url = os.getenv("LIFF_URL", "")
        if liff_url:
            await reply_to_line(reply_token, f"เปิดแอป: {liff_url}")
        else:
            await reply_to_line(reply_token,
                "🎙 ส่งข้อความเสียงมาในแชทได้เลยค่ะ\n"
                "(กดค้างที่ปุ่มไมค์ แล้วพูดสิ่งที่ต้องการบันทึก)")
    else:
        print(f"⚠️  Unknown postback action: {action}")


async def _postback_list(user_id: str, reply_token: str):
    """แสดงรายการล่าสุด"""
    intents = db.get_recent_intents(user_id, limit=5)
    if not intents:
        await reply_to_line(reply_token,
            "ยังไม่มีรายการที่บันทึกไว้ค่ะ\n"
            "ลองส่งข้อความเสียงมาได้เลยค่ะ 🎙")
        return

    lines = ["📋 รายการล่าสุด:\n"]
    for i, intent in enumerate(intents, 1):
        text = intent.get("final_text") or "(ไม่มีข้อความ)"
        date = str(intent.get("created_at", ""))[:10]
        lines.append(f"{i}. {text}  [{date}]")

    await reply_to_line(reply_token, "\n".join(lines))


async def _postback_help(reply_token: str):
    """แสดงวิธีใช้"""
    await reply_to_line(reply_token,
        "🤖 วิธีใช้น้องเก็บให้\n\n"
        "🎙 บันทึกเสียง\n"
        "กดค้างปุ่มไมค์แล้วพูดสิ่งที่ต้องการจำ\n\n"
        "✏️ แก้ไข\n"
        "ถ้าระบบจับเสียงผิด พูดว่า\n"
        "\"ไม่ใช่ หมายถึง [ข้อความที่ถูกต้อง]\"\n\n"
        "📋 ดูรายการ\n"
        "กดปุ่ม \"ดูรายการ\" ในเมนูด้านล่าง\n\n"
        "📊 สรุปวันนี้\n"
        "ดูสิ่งที่บันทึกไว้ทั้งหมดในวันนี้"
    )


async def _postback_correct_last(reply_token: str):
    """แจ้งให้ user ส่งเสียงแก้ไข"""
    await reply_to_line(reply_token,
        "✏️ จะแก้ไขรายการล่าสุดให้ค่ะ\n\n"
        "กรุณาส่งข้อความเสียง โดยพูดว่า:\n"
        "\"ไม่ใช่ หมายถึง [ข้อความที่ถูกต้อง]\""
    )


async def _postback_delete_last(user_id: str, reply_token: str):
    """ลบ intent ล่าสุด (soft-delete → status='cancelled')"""
    intents = db.get_recent_intents(user_id, limit=1)
    if not intents:
        await reply_to_line(reply_token, "ไม่มีรายการที่จะลบค่ะ")
        return

    last = intents[0]
    text = last.get("final_text") or "(ไม่มีข้อความ)"
    cancelled = db.cancel_intent(user_id, last["id"])

    if cancelled:
        await reply_to_line(reply_token, f"🗑 ลบรายการแล้วค่ะ:\n\"{text}\"")
    else:
        await reply_to_line(reply_token, "ไม่สามารถลบได้ค่ะ กรุณาลองใหม่")


async def _postback_summary(user_id: str, reply_token: str):
    """สรุปรายการทั้งหมดของวันนี้"""
    intents = db.get_today_intents(user_id)
    if not intents:
        await reply_to_line(reply_token, "วันนี้ยังไม่มีรายการค่ะ 📭")
        return

    lines = [f"📊 สรุปวันนี้ ({len(intents)} รายการ):\n"]
    for i, intent in enumerate(intents, 1):
        text = intent.get("final_text") or "(ไม่มีข้อความ)"
