...
def get_recent_intents(line_user_id: str, limit: int = 5):
    """ดึง intents ล่าสุดของ user (ไม่รวมที่ถูกยกเลิก)"""
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT * FROM intents 
...
def cancel_intent(line_user_id: str, intent_id: int):
    """Soft-delete: ตั้ง status='cancelled' แทนการลบจริง"""
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                UPDATE intents
                ...
                RETURNING *
            """, (intent_id, line_user_id))
            conn.commit()
            row = cur.fetchone()
            return dict(row) if row else None


def get_today_intents(line_user_id: str):
    """ดึง intents ทั้งหมดของวันนี้ (ไม่รวมที่ถูกยกเลิก)"""
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT * FROM intents
